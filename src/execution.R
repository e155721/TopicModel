...
if(1) {
  ...
  for (i in 2:2) {
    ...
    path <- paste(plot_path, file_name, "_used_k_", i, ".pdf", sep = "")
    pdf(file = path)
    plot(lda_result[[1]])
    dev.off()
    #path <- paste(plot_path, file_name, "_unused_top_terms_k_", i, ".txt", sep = "")
    path <- paste(plot_path, file_name, "_used_top_terms_k_", i, ".txt", sep = "")
